# Ninna — termo de participação no beta fechado

*Versão 1 — 06/08/2026*

> **Campos a preencher antes de enviar:** `[SEU NÚMERO]`,
> `[SEU NOME]`, `[E-MAIL]`, `[WHATSAPP]`. Trocar qualquer palavra depois de
> enviado obriga a subir para a versão 2 e recolher o aceite de novo — é o que dá
> sentido à frase fixa do fim. **Apagar este bloco antes de gerar o PDF.**

Oi! Antes de você começar, é justo saber exatamente no que está entrando. É
curto, e está escrito pra ser lido de uma vez só, com bebê no colo.

## O que a Ninna é hoje

Um app em construção, em teste fechado com três mães, até **25 de agosto de
2026**. Não é produto pronto: pode ter erro, pode mudar de um dia pro outro, pode
ficar fora do ar por algumas horas. Se algo te atrapalhar, me avisa — é
exatamente pra isso que este teste existe.

## O que eu guardo

- seu nome e seu e-mail — é o que identifica sua conta;
- o nome do seu bebê, a data de nascimento e, só se você quiser preencher, sexo,
  peso e altura ao nascer e se nasceu prematuro;
- os registros que você fizer: mamada, sono, troca de fralda, humor e sintoma,
  com o horário.

Só isso. Não peço documento, não peço endereço, e não acesso sua agenda, seus
contatos, suas fotos nem sua localização.

## Onde esses dados ficam

Num banco de dados do Supabase, região Brasil (São Paulo). Cada conta enxerga
apenas os próprios dados, e isso é uma regra do banco, não uma promessa de tela:
mesmo que alguém tente, o banco recusa.

Eu, como responsável pela Ninna, tenho acesso administrativo. Não vou abrir seus
registros por curiosidade — e se eu precisar olhar alguma coisa pra resolver um
problema seu, te aviso antes.

Rotina de bebê é dado sensível. Não é vendido, não é compartilhado com ninguém e
não vai para publicidade.

## A Ninna não dá conselho médico

Ela descreve o que você registrou — "por volta de tal horário", "mais ou menos
tanto tempo". Não diz se está certo ou errado, não avalia gravidade e não
substitui consulta. Quem examina seu bebê é o pediatra. Se você estiver
preocupada com alguma coisa, confie no seu instinto e fale com ele.

## Como sair e apagar tudo

Me manda uma mensagem no WhatsApp ([SEU NÚMERO]) dizendo "quero apagar meus
dados".

- confirmo o recebimento em até **1 dia**;
- apago tudo em até **2 dias**: sua conta, seu bebê e todos os registros.

Não precisa dar motivo, e eu não vou perguntar nem tentar te convencer a ficar.
Uma ressalva honesta: o serviço que hospeda o banco guarda backups automáticos
por alguns dias e eles expiram sozinhos — depois disso não sobra nada em lugar
nenhum.

## O que acontece no fim do teste, em 25/08/2026

Por padrão, **eu apago tudo**. Se você quiser continuar usando e manter seus
registros, é só me dizer — mas isso precisa ser dito: o silêncio significa
apagar.

## Pra aceitar

Se estiver tudo certo pra você, me responde aqui no WhatsApp com esta frase:

> Li o termo versão 1 e aceito participar do beta da Ninna.

Guardo essa mensagem como registro do seu aceite. Você pode desistir quando
quiser, sem explicação.

— [SEU NOME], quem faz a Ninna
[E-MAIL] · [WHATSAPP]
